                ...
                <div v-for="stepl in vSteppers" :key="stepl.title" :name="nameId('divco', stepp.id, stepl.id)" >
                    ...
                    <v-stepper-content v-bind:step="stepl.id" :name="nameId('stepci', stepp.id, stepl.id)">
                        <v-card class="mb-5" height="600px">
                            ...
                            <v-container>
                            <v-row :name="nameId('row', stepl.id, 0)">
                            <v-col cols="12" v-for="a in answers" :key="a.id" :name="nameId('col', stepl.id, a.id)">
                              <components v-if="a.question.useText" v-model="a.text" :is="a.question.type" :id="compId(a.question.type, a.id)" :title="a.question.title" :length="a.question.length" :items="a.question.items" />
                              <components v-if="!a.question.useText" v-model="a.value" :is="a.question.type" :id="compId(a.question.type, a.id)" :title="a.question.title" :length="a.question.length" :items="a.question.items"/>
                            </v-col>
                            </v-row>
                            </v-container>
                        </v-card>
                     ...
                     <stepper-content step="2" :name="nameId('stepca', stepp.id, stepl.id)" > 
                        <!--v-card>
                                        ANSWERS
                                        <v-container>
