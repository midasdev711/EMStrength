...
const proteinData = shortMonth.map(m => {
  return {
    month: m,
...
const carbohydratesData = shortMonth.map(m => {
  return {
    month: m,
...
const fatData = shortMonth.map(m => {
  return {
    month: m,
    "Fat": Math.floor(Math.random() * 100) + 20
  }
})
...
export { proteinData, carbohydratesData, fatData, campaignData, locationData, StackData, SinData }
